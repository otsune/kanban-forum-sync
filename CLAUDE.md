...
- **`i18n.py`** — Locale data and tag table generation (`FORUM_SYNC_LANG` driven). Provides status/tag mappings, admin guide messages, and forum guidelines imported by `syncer.py`.
...
- **Attachments**: Files on a thread message (non-bot) → `_sync_attachment()` calls `KanbanBridge.attach_url()` → `kanban_attach_url`, which has Hermes core fetch the Discord CDN URL server-side (SSRF-guarded via `tools/url_safety.py`) and store it as a real `task_attachments` row (25 MB cap, `KANBAN_ATTACHMENT_MAX_BYTES` in `hermes_cli/kanban_db.py`). This lands via upstream [NousResearch/hermes-agent#36019](https://github.com/NousResearch/hermes-agent/pull/36019) (merged via #65698; see `docs/plans/ATTACHMENT_TOOLSET_PR_PLAN.md` for history). Oversize attachments, and any attachment `kanban_attach_url` permanently rejects (bad URL, expired CDN link, etc.), fall back to posting the Discord URL as a `kanban_comment` link instead — this keeps the cursor moving instead of retrying forever. Only a transient dispatch failure holds the cursor back for retry. Attachments share the per-message `last_message_id` cursor with comments — a message advances the cursor only after all its attachments **and** its text body sync successfully (at-least-once; a partial failure re-runs the whole message next cycle).
