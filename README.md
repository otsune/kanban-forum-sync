...
- **Hermes agent tools** — `kanban_forum_sync_status` / `kanban_forum_sync_resync` を LLM から呼び出し可能
- **CLI サブコマンド** — `hermes kanban-forum-sync status|start|stop|sync`
- **セッション内スラッシュコマンド** — `/kanban-forum-sync status|sync|start|stop`
- **タグ変更通知** — Discord 側のステータスタグ変更で Kanban を更新した場合、`ctx.inject_message()` で会話へ通知
...
2. チャンネルに以下のステータスタグを作成（名前を正確に一致）:

| タグ名 (en) | タグ名 (ja) | 対応 Kanban Status |
|---|---|---|
| 🩺 Triage | 🩺 トリアージ | `triage` |
| 📝 Todo | 📝 未着手 | `todo` |
| ✅ Ready | ✅ 着手可能 | `ready` |
| 🔄 Running | 🔄 進行中 | `running` |
| 🚧 Blocked | 🚧 停滞中 | `blocked` |
| 🎉 Done | 🎉 完了 | `done`, `archived` |

`Scheduled` / `予定済み` と `Review` / `レビュー中` は Discord 表示用タグとして作成されますが、Kanban に存在しないステータスのため、それぞれ `ready` / `running` に正規化されます。`Backlog` / `バックログ` は `triage` に正規化されます。

3. Forum チャンネルの ID を取得 → `FORUM_SYNC_CHANNEL_ID`（未設定でも自動解決可）

### 3. 環境変数の設定

`plugin.yaml` の `requires_env` は install 時にプロンプトされる必須値だけを含みます。以下の任意値は必要な場合だけ `.env` または Hermes の `config.yaml` に設定してください。

| Variable | Required | Description |
|---|---|---|
| `FORUM_SYNC_BOT_TOKEN` | Yes (or `DISCORD_BOT_TOKEN`) | Discord Bot token |
| `FORUM_SYNC_CHANNEL_ID` | No | Forum channel snowflake; auto-discovered or created if unset |
...
| `HERMES_KANBAN_DB` | No | Kanban SQLite DB path. Hermes core resolution also honors `HERMES_KANBAN_BOARD` and the active board |
...
export FORUM_SYNC_DEFAULT_ASSIGNEE="your_profile"     # Forum 新規スレッドから Kanban 作成する場合に必要
```
...
When the plugin is enabled, Hermes exposes these tools in the `kanban_forum_sync` toolset:

| Tool | Arguments | Description |
|---|---|---|
| `kanban_forum_sync_status` | none | Returns watcher state, channel id, last sync time, counters, and last error as JSON |
| `kanban_forum_sync_resync` | `mode`: `incremental` or `full` | Runs one sync cycle immediately. `full` re-runs `initial_sync()` without clearing `sync_map.json` |
...
From a Hermes CLI or gateway session:
...
When a Discord Forum status tag changes and `_sync_forum_tags()` successfully updates the matching Kanban task, the plugin calls `ctx.inject_message(..., role="user")` to notify the active agent conversation. If the gateway has no active CLI reference or message injection is unavailable, the failure is debug-logged and the sync continues.
...
Forum 側の補助タグは Kanban の正式ステータスへ正規化されます: `Backlog` / `バックログ` → `triage`, `Scheduled` / `予定済み` → `ready`, `Review` / `レビュー中` → `running`。
...
├── __init__.py          # register(ctx), CLI/スラッシュコマンド, ツール登録, 起動フック
├── schemas.py           # Hermes agent tool schemas
├── tools.py             # Hermes agent tool handlers
├── service.py           # Runtime singleton/context accessors
├── syncer.py            # コア同期エンジン
...
- **Agent surface** ✅ — Hermes tools, `/kanban-forum-sync`, tag-change conversation notification
- **Phase 4** 📋 — 複数 Forum チャンネル対応
