...
1. Discord サーバーに Forum チャンネルを作成
2. チャンネルに以下の8つのタグを作成（名前は正確に一致させること）:
...
3. Forum チャンネルの ID を取得（Discord 設定 → 詳細 → チャンネル ID をコピー）

### 3. 環境変数の設定

```bash
export FORUM_SYNC_BOT_TOKEN="your_discord_bot_token"
export FORUM_SYNC_CHANNEL_ID="your_forum_channel_id"
